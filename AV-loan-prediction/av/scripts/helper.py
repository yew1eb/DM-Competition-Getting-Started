...
def get_mask(dataset, train_size=0.7):
    ...
    loantrain, loantest = train_test_split(range(dataset.shape[0]), train_size=train_size)
...
def vectorizer(train_df, test_df):
    ...
    train = list(train_df_copy.T.to_dict().values())
    test = list(test_df_copy.T.to_dict().values())
